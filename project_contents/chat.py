...
def home():
    ...
    st.empty()
    st.title('CS6440 - Team 2')

    st.markdown("""
    <div style='padding: 20px; border: 1px solid #ccc; border-radius: 10px;'>
        <h2>What do we do? </h2>
        <p>We predict if a particular outbreak has the possibility to turn into an epidemic or a pandemic based on existing markers from population data from FHIR servers and other data sources. </p>
        <p> We also perform analytics to find out which segment of the population is more prone to a particular disease and we will provide the services of an Gen AI-powered chatbot to help users learn about these trends and the predictions better. </p>
    </div>
    ...
    trends = os.path.join(os.path.dirname(__file__), 'Plots/homepage/trends.png')
    col1.header("Trend Analysis on FHIR Data")
    col1.image(trends, use_column_width=True)

    trends = os.path.join(os.path.dirname(__file__), 'Plots/homepage/predictions.png')
    col2.header("ML Prediction on FHIR Data")
    col2.image(trends, use_column_width=True)

    trends = os.path.join(os.path.dirname(__file__), 'Plots/homepage/ai-buddy.png')
    col3.header("HealthBuddy Chat Bot")
    col3.image(trends, use_column_width=True)
    ...
    <ol>
        <li><b>Trend Analysis of FHIR data :</b> Here we have the the trend analysis conducted on high-risk patients, focusing on the top three conditions observed in a synthetic COVID-19 dataset generated through the Synthea utility. The graphs illustrate the death counts, demographic analysis based on race, ethnicity, and gender, disease condition trends, and the top 10 symptoms observed among high-risk patients. For further insights, please refer to the accompanying video and white paper.</li>
        <li><b>ML Prediction on FHIR data : The ML risk prediction model predicts the risk of mortality from COVID 19, given the patient's profile including history of past diseases, demographics, etc. It classifies the patient's risk in two categories: high risk and low risk. The model has been trained on data from FHIR and Synthea.</b> </li>
        <li><b>HealthBuddy Chat Bot : </b> The Generative AI chat bot uses zephyr 7b model, it is trained on Covid FAQ’s, trend Analysis and ML prediction output.</li>
    </ol>

    <p>The data source for all of the above was retrieved from bulk fhir data servers through a custom built pipeline where we managed to retrieve 10000 records, combine it with synthea data and clean it for the above purposes</p>
    </div>
        """, unsafe_allow_html=True)
...
def makeprediction():
    ...
    if uploaded_file_1 is not None and uploaded_file_2 is not None:
        ...
        if output_file_name:
            ...
            selected_columns = ['id', 'risk', 'pred']
            df_selected = merged_df[selected_columns]
            df_renamed = df_selected.rename(columns={'id': 'Patient ID', 'risk': 'Actual Risk', 'pred': 'Predicted Risk'})
            st.table(df_renamed)
...
def conversation_chat(query, chain, history):
    query = query + "Do not ask/answer any further questions. Do not add any extra information and give brief and concise answer"
...
def display_chat_history(chain):
    ...
    with container:
        with st.form(key='my_form', clear_on_submit=True):
            user_input = st.text_input("Question:", placeholder="Sample questions are present in the left pane.", key='input')
...
def create_conversational_chain(vector_store):
    ...
    llm = LlamaCpp(
        streaming=True,
        model_path=os.path.join(os.path.dirname(__file__), 'zephyr-7b-beta.Q4_K_S.gguf'),
        temperature=0.8,
        top_p=0.95,
        min_p=0.10,
        verbose=True,
...
def Chat():
    ...
    if os.path.exists(pdf2):
        ...
        st.sidebar.write("NOTE: Due to limitations of compute resources being available on Azure deployment, the chat bot might take upto 10 minutes to respond. The project video contains a better version as we have it locally deployed where compute is not an issue")
        # Create a DataFrame
